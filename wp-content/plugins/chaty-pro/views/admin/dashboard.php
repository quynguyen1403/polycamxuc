            ...
            <div class="flex sm:self-end flex-col sm:flex-row items-baseline sm:items-center space-y-4 sm:space-y-0 sm:space-x-5 mt-5 sm:mt-0">
                ...
                if (count($chaty_widgets)):
                    if (!$this->data_check() && !$this->is_pro()) { ?>
                        <a class="btn bg-transparent text-cht-gray-150 border-cht-gray-150/70 text-base hover:text-cht-gray-150 font-normal rounded-lg hover:bg-cht-gray-150/10" href="<?php echo admin_url("admin.php?page=chaty-upgrade") ?>">
                            <?php esc_html_e('Create New Widget', 'chaty'); ?>
                        </a>
                    <?php } else { ?>
                        <a class="btn bg-transparent text-cht-gray-150 border-cht-gray-150/70 text-base hover:text-cht-gray-150 font-normal rounded-lg hover:bg-cht-gray-150/10" href="<?php echo admin_url("admin.php?page=chaty-widget-settings") ?>">
                            <?php esc_html_e('Create New Widget', 'chaty'); ?>
                        </a>
                    <?php }
                else: ?>
                    <a class="btn bg-transparent text-cht-gray-150 border-cht-gray-150/70 text-base hover:text-cht-gray-150 font-normal rounded-lg hover:bg-cht-gray-150/10" href="<?php echo admin_url("admin.php?page=chaty-app&widget=0") ?>"><?php esc_html_e("Create Widget", "chaty") ?></a>
                <?php endif; ?>
                ...
                <?php if ($this->data_check() && $this->is_pro()) { ?>
                    ...
                    <?php if ($this->is_pro()) {
                        ...
                        ?>
                        <p class="text-base text-cht-primary-100 font-semibold font-primary italic"><?php esc_html_e('Your Pro plan expires on', 'chaty'); ?> <?php esc_html_e(date('F jS, Y', strtotime($active_license))); ?></p>
                    <?php } ?>
                <?php } else if ($this->is_expired()) {
                    $licenseKey = $this->get_token();
                    $expired_on = $this->is_expired()
                    ?>
                    <span class="text-base text-cht-primary-100 font-semibold font-primary italic">Your pro plan has expired on <?php echo esc_attr(date('F jS, Y', strtotime($expired_on))) ?></span>
                ...
                <?php } else {
                    ...
                    <?php } else { ?>
                        <p class="text-base text-cht-primary-100 font-semibold font-primary italic"><?php printf(esc_html__("Your pro plan is valid until %s", "chaty"), date('F jS, Y', strtotime($active_license))) ?></p>
                    <?php } ?>
                <?php }//end if
                ?>
            </div>
                                ...
                                <tr id="widget_<?php echo esc_attr($widget['index']) ?>" data-widget="<?php echo esc_attr($widget['index']) ?>" data-nonce="<?php echo esc_attr($widget['nonce']) ?>">
                                    ...
                                    <?php if ($date_status) { ?>
                                        <?php if (!empty($widget['created_on'])) {?>
                                            <td class="border-t bg-white py-3.5 text-cht-gray-150 font-primary text-sm text-left px-5 border-r border-cht-gray-50"><?php echo esc_attr(date("F j, Y", strtotime($widget['created_on']))) ?></td>
                                        <?php } else { ?>
                                            <td>&nbsp;</td>
                                        <?php } ?>
                                    <?php } ?>
