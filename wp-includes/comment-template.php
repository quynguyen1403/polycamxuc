...
function comment_form_title( $no_reply_text = false, $reply_text = false, $link_to_parent = true ) {
	...
	if ( false === $reply_text ) {
		...
		$reply_text = __( 'Để lại câu trả lời cho %s' );
	}
	...
	if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		...
		$fields['cookies'] = sprintf(
			...
			sprintf(
				...
				__( 'Lưu thông tin cho bình luận tiếp theo' )
			)
		);
	...
	$defaults = array(
		...
		'must_log_in'          => sprintf(
			...
			sprintf(
				...
				__( 'Bạn phải <a href="%s">đăng nhập</a> để bình luận bài viết.' ),
		...
		'logged_in_as'         => sprintf(
			...
			sprintf(
				...
				__( 'Đăng nhập với tên %1$s. <a href="%2$s">Chỉnh sửa hồ sơ</a>. <a href="%3$s">Đăng xuất?</a>' ),
				$user_identity,
				get_edit_user_link(),
				/** This filter is documented in wp-includes/link-template.php */
		...
		'comment_notes_before' => sprintf(
			...
			sprintf(
				...
				__( 'Địa chỉ email của bạn sẽ được bảo mật.' )
			),
			$required_text
		),
		'comment_notes_after'  => '',
		...
		/* translators: %s: Author of the comment being replied to. */
		'title_reply_to'       => __( 'Để lại câu trả lời cho %s' ),
