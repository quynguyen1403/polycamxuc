...
class RTSimpleLikes
{
    ...
    public function get_likes_button($post_id, $is_comment = NULL)
    {
		...
		$title_like = esc_html__('Thích', 'gostudy-core');
        $title_unlike = esc_html__('Bỏ thích', 'gostudy-core');
        if ($this->gostudy_already_liked($post_id, $is_comment)) {
            $class = ' liked';
            $title = esc_html__('Bỏ thích', 'gostudy-core');
            $icon = $icon_full;
        } else {
            $class = '';
            $title = esc_html__('Thích', 'gostudy-core');
            $icon = $icon_empty;
        }
    ...
    public function init()
    {
        ...
        if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
            exit( esc_html__('Không được phép', 'gostudy-core') );
        }
