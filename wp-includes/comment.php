...
function wp_handle_comment_submission( $comment_data ) {
	...
	if ( get_option( 'require_name_email' ) && ! $user->exists() ) {
		if ( '' == $comment_author_email || '' == $comment_author ) {
			return new WP_Error( 'require_name_email', __( '<strong>Lỗi:</strong> Vui lòng điền vào các trường bắt buộc.' ), 200 );
		} elseif ( ! is_email( $comment_author_email ) ) {
			return new WP_Error( 'require_valid_email', __( '<strong>Lỗi:</strong> Vui lòng nhập một địa chỉ email hợp lệ.' ), 200 );
		}
	}
	...
	if ( '' === $comment_content && ! $allow_empty_comment ) {
		return new WP_Error( 'require_valid_comment', __( '<strong>Lỗi:</strong> Vui lòng nhập bình luận của bạn.' ), 200 );
	}
