...
<div class="container">
    ...
    <header class="flex items-center justify-between flex-wrap py-5 space-y-3 sm:space-y-0">
        ...
        <h2 class="text-3xl font-primary text-cht-gray-150 font-semibold">Widget Analytics</h2>
    </header>
            ...
            <form action="<?php echo admin_url("admin.php") ?>" method="get" id="analytics_form">
                ...
                <div class="date-options mt-5" id="date_options" style="display: <?php echo ($search_for == "custom") ? "block" : "none" ?>">
                    <div class="date-option">
                        <label class="font-primary text-cht-gray-150 text-base" for="start_date">Start Date</label>
                        <input type="text" name="start_date" id="start_date" readonly value="<?php echo esc_attr($start_date) ?>" />
                    </div>
                    <div class="date-option">
                        <label class="font-primary text-cht-gray-150 text-base" for="end_date">End Date</label>
                        <input type="text" name="end_date" id="end_date" readonly value="<?php echo esc_attr($end_date) ?>" />
                    </div>
                    <div class="date-option">
                        <button type="submit" class="btn py-3 search-btn rounded-sm mt-3 lg:mt-0 ">Search</button>
                    </div>
                </div>
            </form>
        </div>
                ...
                <div class="analytics-record">
                    ...
                    <div class="analytics-table">
                        <table class="border-separate w-full rounded-lg border border-cht-gray-50" cellspacing="0" cellpadding="0">
                            <thead>
                                <tr>
                                    <th class="rounded-tl-lg text-cht-gray-150 text-sm font-semibold font-primary py-3 px-8 bg-cht-primary-50 text-left" width="34%"><?php esc_html_e('Channel', 'chaty-pro') ?></th>
                                    <th class="text-cht-gray-150 text-sm font-semibold font-primary py-3 px-5 bg-cht-primary-50" width="22%"><?php esc_html_e('Visitors', 'chaty-pro') ?></th>
                                    <th class="text-cht-gray-150 text-sm font-semibold font-primary py-3 px-5 bg-cht-primary-50" width="24%"><?php esc_html_e('Unique Clicks', 'chaty-pro') ?> 
                                    ...
                                    <th class="rounded-tr-lg text-cht-gray-150 text-sm font-semibold font-primary py-3 px-2 bg-cht-primary-50" width="20%"><?php esc_html_e('Click-rate', 'chaty-pro') ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td class="bg-white py-3.5 px-8 text-cht-gray-150 font-primary text-sm text-left border-t border-r" data-title="<?php esc_html_e('Channel', 'chaty-pro') ?>">
                                        <?php if ($has_icon) { ?>
                                            <span class="img-icon svg-main-icon-<?php echo esc_attr($i_count); ?>">
                                                <?php if ($has_image) { ?>
                                                    <img src="<?php echo esc_url($image_url) ?>" />
                                    ...
                                    <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t border-r" data-title="<?php esc_html_e('Visitors', 'chaty-pro') ?>"><?php echo esc_attr($no_of_views) ?></td>
                                    <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t border-r" data-title="<?php esc_html_e('Unique Clicks', 'chaty-pro') ?>"><?php echo esc_attr($no_of_clicks) ?></td>
                                    <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t" data-title="<?php esc_html_e('Click-rate', 'chaty-pro') ?>"><?php echo (!empty($no_of_views)) ? number_format(($no_of_clicks * 100 / $no_of_views), 2, ".", ",")."%" : "-"; ?></td>
                                </tr>
                                <?php foreach ($record['channels'] as $channel) {
                                    if (isset($socials[$channel['channel_slug']])) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t border-r" data-title="<?php esc_html_e('Visitors', 'chaty-pro') ?>"><?php echo esc_attr($channel['no_of_views']) ?></td>
                                            <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t border-r" data-title="<?php esc_html_e('Unique Clicks', 'chaty-pro') ?>"><?php echo esc_attr($channel['no_of_clicks']) ?></td>
                                            <td class="bg-white py-3.5 px-5 text-cht-gray-150 font-primary text-sm text-center border-t" data-title="<?php esc_html_e('Click-rate', 'chaty-pro') ?>"><?php echo (!empty($channel['no_of_views'])) ? number_format(($channel['no_of_clicks'] * 100 / $channel['no_of_views']), 2, ".", ",")."%" : "-"; ?></td>
                                        </tr>
                                        <?php
                                        $i++;
                                    }//end if
        ...
        <div class="trigger-option-block">
            <div class="reset-analytics-btn">
                ...
                <label class="chaty-switch" for="collect_analytics_data">
                    ...
                    <span class="font-primary text-cht-gray-150">Collect Analytics Data</span>
                </label>
                <div class="clearfix"></div>
                <?php if (!empty($records)) { ?>
                    <a class="btn text-base bg-cht-red/90 hover:bg-cht-red py-2.5 justify-center rounded-lg mt-5 font-primary border-cht-red shadow-2xl shadow-cht-red/60 w-64" href="javascript:;">Reset Data</a>
                <?php } ?>
            </div>
        </div>
    </div>
...
<div class="chaty-popup-form" id="clear-data" style="display: none;">
    ...
    <div class="chaty-popup-content py-8">
        <div class="popup-description text-center font-primary text-base text-cht-gray-150 pb-7">Are you sure you want to reset all data?<br/>All your analytics data will be deleted</div>
        <form action="<?php echo admin_url("admin.php?page=") ?>" method="get">
            <div class="select-field-btn chaty-popup-btns text-center flex items-center space-x-3">
                ...
                <button type="submit" class="btn w-full text-base bg-cht-red/90 hover:bg-cht-red py-2.5 justify-center rounded-lg font-primary border-cht-red shadow-2xl shadow-cht-red/60 popup-form-reset-btn">Reset</button>
...
<div class="chaty-popup-form" id="analytics-popup" style="display: none;">
    ...
    <div class="chaty-popup-content py-10">
        <div class="popup-description text-center font-primary text-cht-gray-150 text-xl">Chaty Analytics was turned off</div>
        <div class="select-field-btn chaty-popup-btns text-center">
            <a href="javascript:;" class="popup-form-cancel-btn btn text-base bg-cht-red/90 hover:bg-cht-red py-2.5 justify-center mt-3 rounded-lg mx-auto font-primary border-cht-red shadow-2xl shadow-cht-red/60 w-40">Close</a>
        </div>
    </div>
</div>
