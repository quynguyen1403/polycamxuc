    ...
    class Gostudy_Get_Page_Title
    {
        ...
        public function get_title_text()
        {
            if (is_home() || is_front_page()) {
                ...
                $title = single_term_title('', false) . esc_html__(' Thẻ', 'gostudy');
            } elseif (is_date()) {
                $title = get_the_time('F Y');
            } elseif (is_author()) {
                $title = esc_html__('Tác giả:', 'gostudy') . ' ' . get_the_author();
            } elseif (is_search()) {
                $title = esc_html__('Tìm kiếm', 'gostudy');
            } elseif (function_exists('learn_press_is_search') && learn_press_is_search()) {
				$title = esc_html__('Tìm kiếm khóa học', 'gostudy');
			} elseif (is_404()) {
				...
				switch (true) {
					...
					case (learn_press_is_courses()):
                        $title = esc_html__('Lưu trữ', 'gostudy');
                ...
                if (function_exists('is_shop')
                    ...
                    $title = esc_html__('Lữu trữ', 'gostudy');
                }
            } else {
                global $post;
                if (!empty($post)) {
                    ...
                    if ('post' == get_post_type($post)) {
                        ...
                        $title = Gostudy::get_option('blog_title_conditional')
                            ? esc_html__('Bài viết trên blog', 'gostudy')
                    ...
                    $title = esc_html__('Không có bài viết nào', 'gostudy');
                }
            }
