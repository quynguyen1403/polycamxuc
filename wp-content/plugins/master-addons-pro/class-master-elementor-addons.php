	...
	final class Master_Elementor_Addons
	{
					...
					foreach ($jltma_all_addons as $key =>  $widget) {
						if (isset($activated_widgets[$widget['key']]) && $activated_widgets[$widget['key']] == true) {
							...
							if (ma_el_fs()->can_use_premium_code__premium_only()) {
								$widget_path = $widget['is_pro'] ? JLTMA_PRO_ADDONS : JLTMA_ADDONS;
				...
				foreach ($blogs as $blog_id) {
					...
					foreach (JLTMA_Addon_Extensions::$jltma_extensions['jltma-extensions']['extension'] as $extensions) {
						if (isset($activated_extensions[$extensions['key']]) && $activated_extensions[$extensions['key']] == true) {
							...
							if (ma_el_fs()->can_use_premium_code__premium_only()) {
								$extensions_path = (isset($extensions['is_pro']) && $extensions['is_pro']) ? JLTMA_PRO_EXTENSIONS : JLTMA_PATH . '/inc/modules/';
								$extensions_file = $extensions_path . $extensions['key'] . '/' . $extensions['key'] . '.php';
							} else {
		...
		public function jltma_admin_notice_missing_main_plugin()
		{
			...
			if ($this->is_elementor_activated()) {
				...
				$message = /* translators: 1: strong start tag, 2: strong end tag. */ sprintf(__('<b>Master Addons</b> requires %1$s"Elementor"%2$s plugin to be installed and activated. Please install Elementor to continue.', 'master-addons' ), '<strong>', '</strong>');
		...
		public function jltma_admin_notice_minimum_elementor_version()
		{
			...
			$message = /* translators: 1: Plugin name 2: Elementor 3: Required Elementor version */ sprintf(
		...
		public function jltma_admin_notice_minimum_php_version()
		{
			...
			$message = /* translators: 1: Plugin name 2: PHP 3: Required PHP version */ sprintf(
				esc_html__('"%1$s" requires "%2$s" version %3$s or greater.', 'master-addons' ),
				'<strong>' . esc_html__('Master Addons for Elementor', 'master-addons' ) . '</strong>',
		...
		{
			add_filter('plugins_api_result', [$this, 'jltma_plugins_api_result'], 10, 3);
		...
		{
			remove_filter('plugins_api_result', [$this, 'jltma_plugins_api_result'], 10, 1);
