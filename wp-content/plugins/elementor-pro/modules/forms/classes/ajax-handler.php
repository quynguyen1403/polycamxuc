...
class Ajax_Handler {
	...
	public static function get_default_messages() {
		return [
			self::SUCCESS => esc_html__( 'Thư đã được gửi thành công.', 'elementor-pro' ),
			self::ERROR => esc_html__( 'Đã xảy ra lỗi. Vui lòng liên hệ qua fanpage Poly Cảm Xúc', 'elementor-pro' ),
			self::FIELD_REQUIRED => esc_html__( 'Trường này là bắt buộc.', 'elementor-pro' ),
			self::INVALID_FORM => esc_html__( 'Phát hiện sự cố. Biểu mẫu chưa được gửi đi.', 'elementor-pro' ),
			self::SERVER_ERROR => esc_html__( 'Lỗi máy chủ. Gửi không thành công.', 'elementor-pro' ),
			self::SUBSCRIBER_ALREADY_EXISTS => esc_html__( 'Người đăng ký đã tồn tại.', 'elementor-pro' ),
		];
	}
	...
	public static function get_default_message( $id, $settings ) {
		...
		return isset( $default_messages[ $id ] ) ? $default_messages[ $id ] : esc_html__( 'Không xác định', 'elementor-pro' );
	}
	...
	public function send() {
		...
		if ( current_user_can( 'edit_post', $post_id ) && ! empty( $this->messages['admin_error'] ) ) {
			$this->add_admin_error_message( esc_html__( 'Thông báo này không hiển thị cho khách truy cập trang web.', 'elementor-pro' ) );
