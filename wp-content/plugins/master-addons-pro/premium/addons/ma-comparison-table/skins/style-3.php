...
<div class="jltma-comparison-table jltma-cmpt-style-3">
    <ul><?php
    ...
    <table>
        <tbody>
            <tr class="jltma-cmpt-header">
                <td class="jltma-cmpt-features-heading"><?php esc_html_e($settings['jltma_cmpt_feature_heading']); ?></td>
                <?php for ($i = 1; $i <= $settings['jltma_cmpt_product_count']; $i++) { ?>
                    <td class="jltma-cmpt-product-heading jltma-cmpt-product-<?php esc_attr_e($i); ?>">
                        <div class="jltma-cmpt-count-box">
                            <div class="jltma-cmpt-count"><?php esc_html_e($i); ?></div>
                        </div>
            ...
            <tr>
                ...
                <?php for ($i = 1; $i <= $settings['jltma_cmpt_product_count']; $i++) { ?>
                    <td class="jltma-cmpt-feature jltma-cmpt-product-price jltma-cmpt-product-<?php esc_attr_e($i); ?>">
                        <?php esc_html_e($settings['jltma_cmpt_product_price_' . $i]);
                        if (!empty($settings['jltma_cmpt_product_link_' . $i]['url'])) {
                            $this->add_link_attributes('button_' . $i . '-link-attributes', $settings['jltma_cmpt_product_link_' . $i]);
                        }
                        $this->add_render_attribute('button_' . $i . '-link-attributes', 'class', 'jltma-cmpt-product-btn'); ?>
                        <div class="jltma-cmpt-feature-button jltma-cmpt-product-<?php esc_attr_e($i); ?>">
                            <?php if ($settings['jltma_cmpt_button_text_' . $i] !== '') { ?>
                                <a <?php echo $this->get_render_attribute_string('button_' . $i . '-link-attributes'); ?>><?php esc_html_e($settings['jltma_cmpt_button_text_' . $i]); ?></a>
                            <?php } ?>
                        </div>
                    </td>
                <?php } ?>
