            ...
            <div id="comment-<?php comment_ID(); ?>" class="stand_comment">
                <div class="thiscommentbody">
                    ...
                    <div class="comment_info">
                        <div class="comment_author_says"><a style="color: #385898"><?php printf('%s', get_comment_author_link()) ?></a></div>
                        <div class="meta-data">
                            <i style="color:#90949c;" class="fa fa-clock"></i><span style="color:#90949c; font-size: 12px; font-style: italic"><?php printf(' %1$s ', get_comment_date()) ?></span>
                            <?php edit_comment_link('<span>('.esc_html__('Chỉnh sửa', 'gostudy').')</span>', '  ', '') ?>
                        </div>
                    </div>
                    <div class="comment_content">
                        <?php if ($comment->comment_approved == '0') : ?>
                            <p style="font-style: italic"><?php esc_html_e('(Bình luận của bạn đang chờ kiểm duyệt.)', 'gostudy'); ?></p>
                        <?php endif; ?>
                        <?php comment_text() ?>
                    </div>
                    <?php comment_reply_link(array_merge($args, array('depth' => $depth, 'before' => '<span class="comment-reply-wrapper">', 'after' => '</span>', 'reply_text' => '' . esc_html__('Trả lời', 'gostudy'), 'max_depth' => $max_depth_comment))) ?>
                </div>
            </div>
            <?php
